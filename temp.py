...
async def perform_steps(query: QueryItem, update_func):
    print("in perform_steps")
    # for step in range(1, 4):
    #     time.sleep(2)  # Simulate a long-running task
    #     update_func(f"Completed Step {step}")

    update_func(f"Performing Step 1/3 Researching the topic  ...")
    # step 1: generate a report

    # report = await create_report(query)
...
"""
    update_func(f"Completed Step 1/3  ")
    print(f"Completed Step 1/3  ")
    
    update_func(f"Performing Step 2/3 Refining the content  ...")
    
    # step 2: create a script 
    # output =  summarize_single_report(report, query.query)
    output =  summarize_single_report(report)
    script = output['script']
    update_func(f"Completed Step 2/3  ")

    # step 3: write the audio to s3 and return the 
    update_func(f"Performing Step 2/3 Refining the content  ...")



@app.post("/start-process/")
async def start_process(query: QueryItem, background_tasks: BackgroundTasks):
    background_tasks.add_task(perform_steps, query, app.sse_update)
...
async def status():
    ...
    async def event_generator():
        while True:
            if hasattr(app, 'sse_message'):
                # print("before yield")
                yield f"data: {app.sse_message}\n\n"
...
app.sse_update = lambda message: setattr(app, 'sse_message', message)